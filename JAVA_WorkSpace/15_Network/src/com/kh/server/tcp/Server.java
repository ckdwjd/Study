	...
	public void serverStart() {
		...
		try {
			...
			while (true) {
				...
				// 3) 접속요청이 오면 수락 후 해당 클라이언트에 대한 소켓 객체 생성
				System.out.println("클라이언트의 요청이 들어올 떄까지 대기중...");
				...
				// 보조스트림을 통해 성능개선
				// 클라이언트로부터 전달된 값을 한줄단위로 읽기위힌 입력용 스트림
				BufferedReader br = new BufferedReader(new InputStreamReader(is));

				// 클라이언트에게 값을 한줄단위로 출력하는 스트림
				PrintWriter pw = new PrintWriter(os, true); // 데이터 출력을 할 때 print, println 메서드를 사용함

				// 7) 스트림을 통해 읽고 쓰기
				while (true) {
					String message = br.readLine(); // 클라이언트로부터 입력이 있을 때까지 블로킹됨
					...
					if (message == null || "exit".equals(message)) {
						...
						pw.flush(); // 현재 스트림에 있는 내용 중 출력이 안된 내용을 강제로 내보내는 메서드
					}
				}
				br.close();
				pw.close();
