...
public class C_Cast {
	...
	public void forceCasting() {
		...
		double d3 = 10.89;
		int i3 = (int)d3; // 10.89 -> 10  실수에서 정수로 형변환이 이루어질 떄 소수점 아래는 버림처리
		System.out.println("i3 : " + i3);
		
		int iNum = 10;
		double dNum = 5.89;
		System.out.println(iNum + dNum);
		//int iSum = iNum + dNum; // iNum이 double 자료형으로 자동형변환됨 10 -> 10.0
								// 결과값으로 15.89 덧셈연산은 잘 수행됨	
								// 다만, 연산결과가 15.89라는 실수값이기 때문에 대입하는 변수의 자료형도
								// 실수자료형이어야 에러가나지 않
		// 해결방법
		//1. 연산결과를 int자료형으로 강제 형변환시킴
		int iSum = (int)(iNum + dNum); //15  실행순서 : 변수생성, 덧셈연산, 강제형변환, 대입연
		System.out.println("iSum : "+iSum);
		
		//2. 덧셈연산 전에 dNum변수를 int 자료형으로 강제형변환
		//	 자동형변환은 작은 바이트 변수가 큰 바이트 변수로 변환되기 때문에 원래는 iNum이 double로 변환되는데
		//	 그 전에 먼저 dNum 강제 형변환 시킨 것
