		...
		<section id="content">
			<div id="menu-container" class="text-center">
				...
				function displayResultTable(id, data) {
					...
					if(data.length > 0) { // mybatis selectList 데이터가 없는경우 빈 list 반환
				...
				<script>
		         	$("#menuRecommendationFrm").submit( e => {
						...
						const frm = $(e.target);
		         		...
		         		$.ajax({
		         			url : `${contextPath}/menus/\${type}/\${tast}`,
		         			success(data) {
		         				console.log(data);
	         ...
	         $("#menuEnrollFrm").submit( e => {
	            ...
	            $.ajax({
	               ...
	               complete(){
	                  ...
	                  e.target.reset(); // 폼내부의 input태그(모든 요소) 초기화
	               }
			...
			<div class="menu-test">
				...
				<form id="menuSearchFrm">
					<input type="text" name="id" placeholder="메뉴번호"
				...
				<form id="menuUpdateFrm">
					...
					<input type="text" name="name" placeholder="메뉴"
						class="form-control" /> <br /> <input type="number" name="price"
						placeholder="가격" step="1000" class="form-control" /> <br />
					<div class="form-check form-check-inline">
						<input type="radio" class="form-check-input" name="type"
							id="put-kr" value="kr" checked> <label for="put-kr"
							class="form-check-label">한식</label>&nbsp; <input type="radio"
			...
			<!--  
				전송방식 "DELETE"로 요청
				성공 시 : 메뉴삭제 성공
				존재하지 않은 메뉴번호를 입력한 경우 -> 해당 메뉴가 존재하지 않습니다
				그외) 메뉴삭제에 실패했습니다
			-->
			<script>
