...
public class MemberDao {
	/*
	 * 기존의 방식 : DAO클래스에 사용자가 요청할 때마다 실행해야하는 SQL문을 자바 소스코드내에	
	 * 			  직접적으로 작성함 ==> 정적 코딩방식(하드코딩)
	 * 
	 * 문제점 : SQL문을 수정해야하는 경우 자바 소스코드를 수정하는 셈
	 * 		  수정된 내용을 반영시키고자 한다면 프로그램을 재구동해야함
	 * 
	 * 해결점 : SQL문들을 별도로 관리하는 외부 파일을 만들어서 실시간으로 파일에 기록된
	 * 		  SQL문들을 동적으로 읽어들여서 실행 => 동적 코딩방식
	 * 
	 */
	 ...
	 *     PreparedStatement로 실행 시 ?(위치홀더) 로 표현된 빈 공간을 실제 값으로 채워주는 과정을 거쳐야한다
	...
	// new MemberDao().XXX(); 할 때 마다 MemberDao객체가 생성되면서
	// xml 파일로부터 Properties객체로 데이터를 읽어들일예정
	public MemberDao() {
		try {
