/*
 * * DDL(DATA DEFINITION LANGUAGE) : 데이터 정의 언너
 * 
 * 오라클에서 제공하는 객체(OBJECT)를 새로이 만들고(CREATE) 구조를 변경하고(ALTER) 구조를 삭제(DELETE)하는 명령문
 * 즉, 구조 자체를 정의하는 언어로 DB관리자, 설계자가 사용함
 * 
 * 오라클에서의 객체(DB를 이루는 구조물들)
 * 테이블(TABLE), 사용자(USER), 함수(FUNCTION), 뷰(VIEW), 시퀸스(SEQUENCE), 인덱스 ...
 * 
 * */
 ...
 * 		  모든 데이터는 테이블을 통해 저장됨(데이터를 조작하려고 하면 테이블을 만들어서 이용해야함)
 * 
 * [표현법]
 * CREATE TABLE 테이블명 (
 * 		컬럼명 자료형,
 ...
 * 
 * <자료형>
 * - 문자 (CHAR(크기) / VARCHAR2(크기)) : 크기 = BYTE
 * 											(숫자, 영문자, 특수문자 = > 1BYTE)
 * 											(한글 => 2 ~ 3BYTE)
 * 			CHAR(바이트 수) : 최대 2000BYTE까지 지정 가능
 * 						    고정길이(적은 값이 들어와도 공백으로 채워 처음 할당한 크기를 유지함)
 * 							주로 들어올 값의 글자수가 정해져있을 경우 사용
 * 							EX) 성별 : 남/여 , M/F
 * 								주민번호 : 6-7 => 14글자 => 14BYTE
 * 
 * 		VARCHAR2(바이트 수) : 최대 4000BYTE까지 지정 가능
 * 							가변길이(적은 값이 들어오면 그 담긴 값에 맞춰서 크기가 줄어듬)
 * 							주로 들어올 값의 글자수가 정해져있지 않은 경우
 * 							EX) 이름, 아이디, 비밀번호, 이메일 ...
 * 
...
CREATE TABLE MEMBER (
	MEMBER_ID VARCHAR2(20), -- 대소문자 구문 X, 낙타등표기법 사용 불가 -> 언더바로 구분
...
-- 테이블 확인방법2 : 데이터 딕션너리 이용
-- 데이터 딕셔너리 : 다양한 객체들의 정보를 저장하고있는 시스템 테이블
SELECT *
FROM USER_TABLES;
-- USER_TABLES : 현재 이 사용자 계정이 가지고있는 테이블들의 전반적인 구조를 확인할 수 있는 데이터 딕셔너리

SELECT *
FROM USER_TAB_COLUMNS; -- 컬럼들 확인하는 방법

/*
 *  컬럼에 코멘트 달기(칼람에 대한 설명)
 * 
 * [표현법]
...
-- 한 행으로 추가(행을 기준으로 추가), 추가할 값을 기술(값의 순서가 중요)
-- INSERT INTO 테이블명 VALUES(첫번쨰 컬럼의 값, 두번쨰 컬럼의 값 ...)
INSERT INTO MEMBER VALUES('user01','pass01','홍길동','99/05/10');

INSERT INTO MEMBER VALUES('user02','pass02','김갑생','1980/10/06');

INSERT INTO MEMBER VALUES('user02','pass02','박말똥',SYSDATE);

INSERT INTO MEMBER VALUES(NULL,NULL,NULL,SYSDATE);
-- 아이디, 비밀번호, 이름, NULL값 존재 X

INSERT INTO MEMBER VALUES('user02','pass02','박말똥',SYSDATE);
-- 중복된 아이디가 존재 X

-- 위의 NULL값이나 중복된 아이디값은 유효하지 않은 값들
-- 유효한 데이터 값을 유지하기 위해서 제약조건을 걸어야 함
...
/*
 * 1. NOT NULL 제약조건
 * 	  해당 컬럼에 반드시 값이 존재하야만 할 경우 사용
 * 	  => 즉, NULL 값이 절대 들오면 안되는 컬럼에 부여하는 제약조건
...
-- 컬럼레벨 방식 : 컬럼명 자료형 제약조건 => 제약조건을 부여하고자 하는 컬럼 뒤에 기술하는 방법
...
VALUES(1, 'user01', 'pass01', '경민', '남', '010-1111-1111','alsrudals@naver.com');

SELECT *
FROM MEM_NOTNULL ;

INSERT INTO MEM_NOTNULL 
VALUES(2, 'user02', NULL, '경민', '남', '010-1111-1111','alsrudals@naver.com');
-- DDL 계정에 MEM_NOTNULL 테이블에 NOT NULL 재약조건이 부여된 컬럼들에 NULL값이 들어갈 수 없어서 오류 발생

INSERT INTO MEM_NOTNULL 
VALUES(2, 'user02', 'pass02', '경민', NULL, NULL,NULL);
...
-- 테이블 레벨방식 : 모든 컬럼을 다 기술하고, 그 이후에 제약조건을 나열
CREATE TABLE MEM_UNIQUE (
	MEM_NO NUMBER NOT NULL,
	MEM_ID VARCHAR2(20) NOT NULL,
	MEM_PWD VARCHAR2(20) NOT NULL,
...
INSERT INTO MEM_UNIQUE mu
VALUES(1, 'user01','pass01','경민','남','010-1111-2222','abc@naver.com');

/*
 * UNIQUE 제약조건에 위배되었으므로 INSERT 실패
 * 
 * - 어느 컬럼에 어느 문제가 있는지 구체적으로 알려주지 않음
 * - DDL.SYS_C007114 : 제약조건의 이름으로만 제약조건 위배를 알려줌
 * - 제약조건 부여 시 직접 제약조건명을 지정해주지 않으면 시스템에서 임의의 제약조건명을 부여해줌
 * 
 * */
...
-- 추가적으로 NULL값을 못 들어오게 ㅎㅏ고싶다면 NOT NULL 제약조건도 같이 걸어주면 됨
...
-- NULL값은 들어가도 된다. 단, 부모테이블에 존재하지 앟는 값이 들어가면 안된다.
